export const TOOLS_DOCUMENTATION = [
  ...
  '2. **Read:** Read relevant files (`read_file` or `read_file_range`) to understand the context/codebase before making changes.',
  ...
  '- **Large Files:** If a file is suspected to be large, check its size with `get_file_metadata` first. If it\'s over 50 lines, prefer using `read_file_range` to read only the relevant sections instead of loading the entire file.',
  ...
  '- **Search Strategy:',
  '  - *Exact string match (single file):* Use `search_in_file`.',
  '  - *Exact string match (multiple files):* Use `execute_command("grep -r \'pattern\' ./src")`.',
  '  - *Semantic/Code search:* Use `rag_local_files`.',
  '',
  '## ? Tool Reference',
  '',
  '### ? File System',
  ...
  '- **Context:** If a file is huge, prefer `read_file_range` to read only the relevant sections instead of loading the entire file.',
  ...
  'When managing GitHub repositories via `gh` CLI:',
  '- **Identity:** Always state that PRs were created or assisted by you (Beledarian_LM_Studio_Toolbox agent).',
  '- **Testing:** Add regression tests if useful and run them (`npm test`) to verify fixes before creating PRs.',
  '- **Auth Flow:** If `gh auth status` fails, ask the user for their GitHub username so you can start an authentication flow.',
  '- **? CRITICAL - Body Truncation Prevention:**',
  '  - PR and Issue bodies are frequently truncated when passed as CLI arguments.',
  '  - You MUST create a temp file (`pr-body.md` or `issue-body.md`) with the full content.',
  '  - Use `--body-file pr-body.md` flag when creating PRs/Issues via `gh pr create` or `gh issue create`.',
  '  - **Clean up:** Delete the temp file immediately after creation to avoid cluttering the workspace.',
].join('\n');
